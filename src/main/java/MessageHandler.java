    ...
    public void showFlower() {
        ...
        for (List<String> names : FlowerLoader.get()) {
            ...
            if (names.size() > 1) {
                ...
                sendMessageWithRowButton(chatId, flowerPrice + ActButton.ORDER.getReplay(),
                        FileLoader.getAllSingleNames(names),
                        AppProperties.NUMBER_BUTTON_IN_ROW,
                        true);
            } else {
                sendPhoto(chatId, names.get(0), flowerPrice + ActButton.ORDER.getReplay(), FileLoader.getAllSingleNames(names));
            }
        }
        ...
        addId(sendMessage(chatId, "Выберете цветок нажатием соответствующей кнопки. " +
