...
public enum QuestionButton {
    ...
    public String getReplay(Goods goods, String firstData, String lastData) {
        return switch (getButtonOnName(name)){
            ...
            case CASH -> "Оплата заказа наличными курьеру. Другие способы оплаты к сожелению не предусмотрены";
            ...
            case DISCOUNTER -> "Уступить хорошему клиенту пару рублей приятно. Но заказ должен быть не менее "
                    + goods.getDiscountSize() + " шт, при этом скидка будет состовлять "
            ...
            case WRAPPER -> " Имеется два вида упаковки в плёнку и в крафт.";
            case CORRECT -> "Откорректировать сформированный и отправленный на упаковку заказ возможено только через обратную связь. "+
